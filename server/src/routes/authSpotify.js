...
const { generatePkcePair, generateState } = require('../utils/pkce');
const { encryptToken } = require('../utils/tokenCrypto');
const { upsertSpotifyUserTokens } = require('../utils/db');
...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    if (!code || !state || !req.session.spotifyAuth) {
      return res.redirect(buildFrontendRedirect({ status: 'error', message: 'Missing code or state' }));
    }
    ...
    const message = callbackError instanceof Error ? callbackError.message : 'Callback failed';
    return res.redirect(buildFrontendRedirect({ status: 'error', message }));
  }
});
